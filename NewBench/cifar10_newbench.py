...
def impulse_noise(x, severity=1):
    c = [.0055, .014, .025, .035, .046][severity - 1]
...
def defocus_blur(x, severity=1):
    c = [(0.35, 0.45), (0.4, 0.65), (0.44, 0.9), (0.89, 1.8), (1.2, 1.9)][severity - 1]
...
def glass_blur1(x, severity=1):
    ...
    c = [(0.0000001,1,1), (0.001,1,1), (0.5,1,1), (0.25,1,2), (0.5,1,2)][severity - 1]
...
def glass_blur(x, severity=1):
    ...
    severity_settings = [
        (0.01, 1, 1, 0.07),  # Severity 1: Minimal blur and swapping
        (0.4,      1, 1, 0.19),  # Severity 2: Slight increase in blur and swapping
        (0.4,        1, 1, 0.38),  # Severity 3: Moderate blur and swapping
        (0.3,       1, 2, 0.2),  # Severity 4: Increased iterations and swapping
        (0.5,        1, 2, 0.45)   # Severity 5: Maximum blur and swapping
    ]
...
def motion_blur(x, severity=1):
    c = [(1,0.8), (6,1.4), (6,2), (9,2.8), (7,3.5)][severity - 1]
...
def zoom_blur(x, severity=1):
    c = [np.arange(1, 1.04, 0.01), np.arange(1, 1.13, 0.01), np.arange(1, 1.22, 0.01),
         np.arange(1, 1.28, 0.01), np.arange(1, 1.40, 0.01)][severity - 1]
...
def snow(x, severity=1):
    c = [(0.015,0.07,1.0,0.1,5,5,0.95),
         (0.09,0.1,1.0,0.1,9,9,0.85),
         (0.09,0.2,1.25,0.2,10,20,0.65),
         (0.1,0.3,1.75,0.3,12,20,0.65),
         (0.2,0.3,1.25,0.4,8,18,0.62)][severity - 1]
...
def frost(x, severity=1):
    c = [(1, 0.2), (1, 0.4), (0.8, 0.5), (0.9, 0.6), (1, 0.68)][severity - 1]
...
def fog(x, severity=1):
    c = [(.4,3), (.75,2.5), (1.05,2), (1.3,2), (1.55,1.75)][severity - 1]
...
def elastic_transform(image, severity=1):
    IMSIZE = 32
    c = [(IMSIZE*0, IMSIZE*0, IMSIZE*0.03),
         (IMSIZE*0.2, IMSIZE*0.2, IMSIZE*0.085),
         (IMSIZE*0.2, IMSIZE*0.2, IMSIZE*0.142),
         (IMSIZE*0.2, IMSIZE*0.2, IMSIZE*0.19),
         (IMSIZE*0.2, IMSIZE*0.2, IMSIZE*0.24)][severity - 1]
...
def pixelate(x, severity=1):
    c = [0.9, 0.69, 0.55, 0.48, 0.43][severity - 1]
