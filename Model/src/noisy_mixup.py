...
def _noise(x, add_noise_level=0.0, mult_noise_level=0.0, sparse_level=0.0):
    ...
    with torch.cuda.device(0):
        if add_noise_level > 0.0:
            var = torch.var(x)**0.5
            add_noise = add_noise_level * np.random.beta(2, 5) * torch.randn(x.shape, device='cuda')
            #torch.clamp(add_noise, min=-(2*var), max=(2*var), out=add_noise) # clamp
            sparse = torch.rand(x.shape, device='cuda')
            add_noise[sparse<sparse_level] = 0
        if mult_noise_level > 0.0:
            mult_noise = mult_noise_level * np.random.beta(2, 5) * (2 * torch.rand(x.shape, device='cuda') - 1) + 1
            sparse = torch.rand(x.shape, device='cuda')
